...
- none
  - ソースコードはそのまま出力されて、依存関係用のコードが追加されている
- development
  - デバッグ用のコメントやソースマップ、モジュール用のeval のコードが追加されている
- production
  - コードが圧縮・最適化され、ファイルサイズが小さくなるが、可読性が大幅に低下している。
...
/**
* BitSetはAbstractWritableSetの具象サブクラス。ある最大値よりも * 小さい非負の整数がメンバーとなるセットに対して、非常に効率的な
* 固定サイズのセットを実装する。
*/
...
// モジュールを使ってコードを記述する。
let s = new BitSet(100); s.insert(10);
s.insert(20);
s.insert(30);
